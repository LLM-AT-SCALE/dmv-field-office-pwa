/* ==========================================================================
   ...
   Ported from legacy-demo/js/pdf-fill.js. The field names, the option indices
   and the positional selection are verified against the real PDF: changing any
   of them silently produces a form the counter cannot accept.
   ========================================================================== */
...
export const REG343_TEMPLATE_URL = '/forms/REG-343_fill-template.pdf';

/* ---- helpers ------------------------------------------------------------ */

function setText(form: PDFForm, name: string, value: FieldValue): void {
  if (value === undefined || value === null || String(value).trim() === '') return;
  try { form.getTextField(name).setText(String(value)); }
  catch { console.warn('[reg343] no text field', name); }
}

/* Spread a string across a run of single-character comb boxes. */
function setComb(form: PDFForm, names: string[], value: FieldValue): void {
  if (!value) return;
  const chars = String(value).toUpperCase().replace(/\s+/g, '');
  names.forEach((n, i) => { if (chars[i]) setText(form, n, chars[i]); });
}

function checkBox(form: PDFForm, name: string, on: boolean): void {
  if (!on) return;
  try { form.getCheckBox(name).check(); }
  catch { console.warn('[reg343] no checkbox', name); }
}
...
function selectByPosition(form: PDFForm, fieldName: string, index: number | null | undefined): void {
...
/* The stored option values are always English (see lib/types.ts) — these
   lookups key off them, which is why they are typed as literal unions. */
const VEHICLE_TYPE_INDEX: Record<string, number> = {
  'Auto': 0, 'Commercial (includes truck or pickup)': 1, 'Motorcycle': 2,
  'Off Highway': 3, 'Trailer Coach': 4
};
const ACQUIRED_FROM_INDEX: Record<string, number> = { 'Dealer': 0, 'Private Party': 1, 'Dismantler': 2, 'Immediate Family Member': 3 };
const METHOD_INDEX: Record<string, number> = { 'Purchase': 0, 'Gift': 1, 'Trade': 2 };
const PLATE_INDEX: Record<string, number> = { 'Surrendered to CA DMV': 0, 'Destroyed': 1, 'Retained': 2, 'Returned to the issuing state': 3 };

/* Look an option up without tripping over an answer that has not been given. */
function indexOf(table: Record<string, number>, value: string | undefined): number | null {
  if (!value) return null;
  const i = table[value];
  return i === undefined ? null : i;
}

const yesNo = (v: string | undefined): number | null => v === 'Yes' ? 0 : v === 'No' ? 1 : null;

/* Split an ISO date into month / day / year parts */
function dateParts(iso: string | undefined): { y: string; m: string; d: string } | null {
  if (!iso) return null;
...
function fillDate(form: PDFForm, iso: string | undefined, mField: string, dField: string, yField: string): void {
...
function fillOdometer(form: PDFForm, reading: string | undefined): void {
  if (!reading) return;
...
export interface FillMeta {
  /* Token number, used to name the file the technician receives. */
  token?: string | null;
}

export interface FillOptions {
  /* Template bytes, read however the caller likes. Supply these on the server,
     where a root-relative URL cannot be fetched. */
  template?: ArrayBuffer | Uint8Array;
  /* Where to fetch the template from when bytes are not supplied. */
  templateUrl?: string;
}
...
async function loadTemplate(options: FillOptions): Promise<ArrayBuffer | Uint8Array> {
  if (options.template) return options.template;
  const url = options.templateUrl || REG343_TEMPLATE_URL;
  const res = await fetch(url);
  if (!res.ok) throw new Error('Could not load the REG 343 form (' + res.status + ')');
  return res.arrayBuffer();
}

export async function fillREG343(
  data: FormData,
  meta?: FillMeta,
  options: FillOptions = {}
): Promise<FilledPdf> {
  const bytes = await loadTemplate(options);
  ...
  selectByPosition(form, 'Type of Vehicle', indexOf(VEHICLE_TYPE_INDEX, d.vehicleType));
  ...
  /* --- Section 6: cost --------------------------------------------------- */
  selectByPosition(form, 'Purchase Gift Trade', indexOf(METHOD_INDEX, d.acquisitionMethod));
  setText(form, 'Text156', d.purchasePrice);
  setText(form, 'Text158', d.giftValue);
  setText(form, 'Text160', d.tradeValue);
  selectByPosition(form, 'Acquired from', indexOf(ACQUIRED_FROM_INDEX, d.acquiredFrom));
  ...
  else selectByPosition(form, 'Expired or will be or were', indexOf(PLATE_INDEX, d.plateDisposition));
...
/* Browser only: fill the form and hand it to the customer as a download. */
export async function downloadREG343(
  data: FormData,
  meta?: FillMeta,
  options: FillOptions = {}
): Promise<string> {
  if (typeof document === 'undefined') {
    throw new Error('downloadREG343 needs a browser; call fillREG343 on the server.');
  }
  const { bytes, filename } = await fillREG343(data, meta, options);
