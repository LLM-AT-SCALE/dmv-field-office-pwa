/* ==========================================================================
   ...
   Ported unchanged from legacy-demo/js/barcode.js.
   ========================================================================== */
...
export const NARROW = 2;   // px per narrow element
export const WIDE   = 5;   // px per wide element — must be 2–3x narrow
export const QUIET  = 20;  // quiet zone; scanners need it to find the symbol

/* Whether every character of `value` can be drawn as Code 39. */
export function canEncodeCode39(value: string): boolean {
  return String(value || '').toUpperCase().split('').every(c => !!CODE39[c]);
}

/* Returns an SVG string encoding `value`, or an empty string if any character
   cannot be represented. */
export function code39SVG(value: string, height?: number): string {
